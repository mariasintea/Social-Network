...
public class PaginatedUserDatabase implements PaginatedRepository<Long, User> {
    String tableName;
    final private String databaseName = "jdbc:postgresql://localhost:5432/socialnetwork";
    final private Validator<User> validator;

    public PaginatedUserDatabase(String tableName, Validator<User> validator)
    {
    ...
    @Override
    public Page<User> findAll(Pageable pageable, Function<User, Boolean> sortingFunction) {
        List<User> list = new ArrayList<>();
        for(User user:this.findAll())
            if (sortingFunction.apply(user))
                list.add(user);
        Paginator<User> paginator = new Paginator<User>(pageable, list);
