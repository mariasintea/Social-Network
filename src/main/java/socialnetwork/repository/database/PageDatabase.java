...
public class PageDatabase implements Repository<Long, Page> {

    String tableName;
    final private String databaseName = "jdbc:postgresql://localhost:5432/socialnetwork";
    final private Validator<Page> validator;

    public PageDatabase(String tableName, Validator<Page> validator)
    {
    ...
    protected String createEntityAsString(Page entity) {
        ...
        if(requestList.equals("''"))
            requestList = "NULL";
        return entity.getId().toString() + ";" + entity.getName() + ";" + entity.getSurname() + ";" + friendsList + ";" + messagesList + ";" + requestList;
    }
    ...
    public Page save(Page entity) {
        ...
        try
        {
            ...
            String row = createEntityAsString(entity);
            String[] args = row.split(";");
            String query = "insert into " + tableName +"(id, name, surname, friends, messages, requests) values('"+ args[0] + "', '" + args[1] +"', '" + args[2] + "', " + args[3] + ", " + args[4]+ ", " + args[5] + ")";
            Statement stmt = con.createStatement();
            int x = stmt.executeUpdate(query);
            if (x == 0)
                throw new Exception("Insert Failed");
